...
export async function sendWelcomeEmail(
  { email: userEmail, firstName }: User,
  ...
  const email: Mailgen.Content = {
    body: {
      ...
      action: {
        instructions:
          "Per verificare il tuo account e impostare una password, clicca il pulsante di seguito:",
        button: {
  ...
  await sendEmail(
    userEmail,
    env.EMAIL_FROM,
    "Benvenuto in Wellness & Nutrition",
    email,
  );
}
...
export async function sendResetEmail(
  ...
  const email: Mailgen.Content = {
    body: {
      ...
      action: {
        instructions:
          "Per ripristinare le credenziali, clicca il pulsante di seguito:",
        button: {
          color: "#22BC66", // Optional action button color
